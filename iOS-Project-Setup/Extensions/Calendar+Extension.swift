...
extension Calendar {
    ...
    func startOfPreviousMonth(_ date: Date) -> Date {
        let previousMonth = Calendar.current.date(byAdding: .month, value: -1, to: date)!
        return self.date(from: self.dateComponents([.year, .month], from: previousMonth))!
    }
    
    func endOfPreviousMonth(_ date: Date) -> Date {
        return self.date(byAdding: DateComponents(month: 1, day: -1), to: self.startOfPreviousMonth(date))!
    }
    
    func startOfNextMonth(_ date: Date) -> Date {
        let nextMonth = Calendar.current.date(byAdding: .month, value: 1, to: date)!
        return self.date(from: self.dateComponents([.year, .month], from: nextMonth))!
    }
    
    func endOfNextMonth(_ date: Date) -> Date {
        return self.date(byAdding: DateComponents(month: 1, day: -1), to: self.startOfNextMonth(date))!
    }
